...
const docs: DocItem[] = [
  ...
  {
    id: 'hosted-security',
    title: 'Hosted MCP Security',
    description: 'Bearer, OIDC, mTLS, IP allowlists, caller identity, key rotation, audit export',
    icon: 'bi-shield-check',
  },
  {
    id: 'hosted-runtime-isolation',
    title: 'Hosted runtime isolation',
    description: 'CPU/memory tiers, optional deny-by-default HTTP egress for tools',
    icon: 'bi-box-seam',
  },
  {
    id: 'keycloak-local-oidc',
    title: 'Keycloak & local OIDC',
    description: 'Docker Compose Keycloak and hosted_security_config for OIDC testing',
    icon: 'bi-hdd-network',
  },
  {
    id: 'hosted-oauth-bff',
    title: 'Hosted OAuth BFF',
    description: 'IdP login via Make MCP, per-server discovery, token endpoint, Cursor mcp.json',
...
export default function Docs() {
  ...
  useEffect(() => {
    ...
    if (fromQuery && DOC_IDS.has(fromQuery)) {
      setSelectedDoc(fromQuery);
    }
    // Intentionally run once on mount so "Back to Docs" is not overridden by ?doc= still in the URL.
