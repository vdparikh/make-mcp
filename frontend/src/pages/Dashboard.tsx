...
import { listServers, listCompositions, createServer, createDemoServer, createBlueprintServer, createMCPAppsLabServer, deleteServer } from '../services/api';
...
export default function Dashboard() {
  ...
  const handleCreateBlueprintServer = async () => {
    try {
      const server = await createBlueprintServer();
      toast.success('MCP Production Blueprint created — tools, resources, prompts, context, and policies.');
      loadServers();
      navigate(`/servers/${server.id}`);
    } catch (error) {
      toast.error('Failed to create blueprint server');
    }
  };

  const handleCreateMCPAppsLabServer = async () => {
    try {
      const server = await createMCPAppsLabServer();
      toast.success('MCP Apps Lab created — open Tools → Test to try each widget (or use MCP Jam).');
      loadServers();
      navigate(`/servers/${server.id}`);
    } catch (error) {
      toast.error('Failed to create MCP Apps Lab server');
    }
  };
  ...
  const renderPrimaryActions = (className = 'dashboard-action-row') => (
    <div className={className}>
      ...
      <button type="button" className="btn btn-outline-primary dashboard-action-btn" onClick={() => navigate('/import/server-json')}>
