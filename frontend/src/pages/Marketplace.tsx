...
import { listMarketplace, getMarketplaceServer, unlistServerFromMarketplace } from '../services/api';
import { useAuth } from '../contexts/AuthContext';
import ConfirmModal from '../components/ConfirmModal';

type InspectorTab = 'tools' | 'resources' | 'prompts' | 'versions' | 'security';

function isMarketplaceOwner(userId: string | undefined, server: Server): boolean {
  return Boolean(userId && server.owner_id && userId === server.owner_id);
}
...
export default function Marketplace() {
  ...
  const { user } = useAuth();
  const [unlistTarget, setUnlistTarget] = useState<Server | null>(null);
  ...
  const handleConfirmUnlist = async () => {
    ...
    const serverId = unlistTarget.id;
    try {
      setUnlisting(true);
      await unlistServerFromMarketplace(serverId);
      ...
      const message =
        typeof err === 'object' && err !== null && 'response' in err
          ...
          : undefined;
      toast.error(message || 'Failed to remove from marketplace');
    } finally {
      setUnlisting(false);
    }
  };
  ...
  return (
    <div className="dashboard">
        ...
        <div className="marketplace-grid">
          {filteredServers.map((server) => (
            <div key={server.id} className="marketplace-card">
              ...
              {isMarketplaceOwner(user?.id, server) && (
                <div
                  className="marketplace-card-owner-strip"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <span style={{ color: 'var(--text-secondary)' }}>
                    <i className="bi bi-person-badge" style={{ marginRight: '0.35rem' }}></i>
                    Your listing
                  </span>
                  <button
                    type="button"
                    className="btn btn-sm btn-outline-danger"
                    onClick={() => setUnlistTarget(server)}
                  >
                    <i className="bi bi-shop-window" style={{ marginRight: '0.25rem' }}></i>
        ...
        >
          ...
          >
            ...
            <div className="marketplace-inspector-header" style={{ 
              ...
              <div className="marketplace-inspector-header-actions" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                {isMarketplaceOwner(user?.id, selectedServer) && (
                  <button
                    type="button"
                    className="btn btn-outline-danger"
                    onClick={() => setUnlistTarget(selectedServer)}
      ...
      <ConfirmModal
        ...
        message={
          unlistTarget
            ? `“${unlistTarget.name}” will be hidden from the public marketplace. You can list it again from the server editor (Versions → publish with “List on marketplace”).`
            : ''
        }
