...
type WizardField = { id: string; label: string; type: 'text' | 'number'; default: string | number };
type RecipeBuild = (values: Record<string, string | number>) => { type: PolicyRuleType; config: Record<string, unknown>; failAction: 'deny' | 'warn' | 'approve' };
...
interface PolicyRecipe {
  ...
  buildRule: RecipeBuild;
}

const policyRecipes: PolicyRecipe[] = [
  {
    id: 'payment_limits',
    name: 'Payment limits',
    description: 'Cap a numeric input field (e.g. amount) at a maximum value.',
    icon: 'bi-currency-dollar',
    policyName: 'Payment Limits',
    policyDescription: 'Deny tool calls when the specified field exceeds the maximum value',
    wizardFields: [
      { id: 'field', label: 'Input field to cap', type: 'text', default: 'amount' },
      { id: 'max_value', label: 'Maximum value', type: 'number', default: 5000 },
    ],
    buildRule: (v) => ({
      type: 'max_value',
      config: { field: String(v.field || 'amount'), max_value: Number(v.max_value) || 5000 },
      failAction: 'deny',
    }),
  },
  {
    id: 'approval_production',
    name: 'Approval for production writes',
    description: 'Require human or manager approval before the tool runs.',
    icon: 'bi-shield-lock',
    policyName: 'Production Write Approval',
    policyDescription: 'Requires approval before execution in production',
    wizardFields: [
      { id: 'message', label: 'Approval message', type: 'text', default: 'Production write requires manager approval' },
    ],
    buildRule: (v) => ({
      type: 'approval_required',
      config: { approval_type: 'human', message: String(v.message || 'Approval required') },
      failAction: 'approve',
    }),
  },
  {
    id: 'weekend_freeze',
    name: 'Weekend freeze',
    description: 'Block tool calls on weekends (Saturday and Sunday).',
    icon: 'bi-calendar-x',
    policyName: 'Weekend Freeze',
    policyDescription: 'Tool calls not allowed on weekend days',
    wizardFields: [
      { id: 'timezone', label: 'Timezone', type: 'text', default: 'America/New_York' },
    ],
    buildRule: (v) => ({
      type: 'time_window',
      config: {
        start_hour: 0,
        end_hour: 24,
        timezone: String(v.timezone || 'America/New_York'),
        weekdays: [1, 2, 3, 4, 5],
      },
      failAction: 'deny',
    }),
  ...
  {
    id: 'allowed_roles',
    name: 'Allowed roles only',
    description: 'Restrict tool access to specific user roles from context.',
    icon: 'bi-people',
    policyName: 'Allowed Roles',
    policyDescription: 'Only users with the specified roles can call this tool',
    wizardFields: [
      { id: 'roles', label: 'Comma-separated roles', type: 'text', default: 'admin, finance_agent' },
    ],
    buildRule: (v) => {
      const rolesStr = String(v.roles || 'admin');
      const roles = rolesStr.split(',').map((s) => s.trim()).filter(Boolean);
      return {
        type: 'allowed_roles',
        config: { roles: roles.length ? roles : ['admin'] },
        failAction: 'deny',
      };
    },
  },
  {
    ...
    description: 'Allow tool calls only during business hours (e.g. 9–17).',
    icon: 'bi-clock',
    policyName: 'Business Hours',
    policyDescription: 'Tool calls allowed only during configured hours',
    wizardFields: [
      { id: 'start_hour', label: 'Start hour (0–23)', type: 'number', default: 9 },
      { id: 'end_hour', label: 'End hour (0–23)', type: 'number', default: 17 },
      { id: 'timezone', label: 'Timezone', type: 'text', default: 'America/New_York' },
    ],
    buildRule: (v) => ({
      type: 'time_window',
      config: {
        start_hour: Number(v.start_hour) ?? 9,
        end_hour: Number(v.end_hour) ?? 17,
        timezone: String(v.timezone || 'America/New_York'),
        weekdays: [1, 2, 3, 4, 5],
      },
      failAction: 'deny',
...
export default function PolicyEditor({ tools, initialToolId, onPolicyUpdated, lockToolSelection }: Props) {
  const [selectedTool, setSelectedTool] = useState<string>('');
  const [policies, setPolicies] = useState<Policy[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [loading, setLoading] = useState(false);
  const [showWizard, setShowWizard] = useState(false);
  const [selectedRecipe, setSelectedRecipe] = useState<PolicyRecipe | null>(null);
  const [wizardValues, setWizardValues] = useState<Record<string, string | number>>({});
  ...
  const openRecipeWizard = (recipe: PolicyRecipe) => {
    const initial: Record<string, string | number> = {};
    recipe.wizardFields.forEach((f) => {
  ...
  const applyRecipe = () => {
    ...
    const built = selectedRecipe.buildRule(wizardValues);
    setPolicyName(selectedRecipe.policyName);
    setPolicyDescription(selectedRecipe.policyDescription);
    setRules([{
      type: built.type,
      config: JSON.stringify(built.config, null, 2),
      failAction: built.failAction,
    }]);
    setShowWizard(false);
    setSelectedRecipe(null);
    setShowForm(true);
    toast.info('Policy built from template — review and create');
  };
    ...
    <div>
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ 
          ...
          <div style={{ marginTop: '0.75rem', fontSize: '0.8rem' }}>
            <a
              href="https://github.com/vdparikh/make-mcp/blob/main/docs/security-best-practices.md#policies"
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: '#38bdf8', textDecoration: 'none' }}
            >
              <i className="bi bi-box-arrow-up-right" style={{ marginRight: '0.25rem' }}></i>
              Read full governance & policies guide
            </a>
          </div>
        </div>
      </div>
      ...
      {selectedTool && !showForm && (
        <div className="card" style={{ marginBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
            Use a recipe and answer a few questions to generate the policy config.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
            {policyRecipes.map((recipe) => (
              <button
                key={recipe.id}
                type="button"
                className="btn btn-secondary"
                style={{ textAlign: 'left', padding: '1rem', height: 'auto' }}
                onClick={() => openRecipeWizard(recipe)}
              >
                <i className={`bi ${recipe.icon}`} style={{ marginRight: '0.5rem', color: 'var(--primary-color)' }}></i>
      ...
      {showWizard && selectedRecipe && (
        <div
          ...
          onClick={() => { setShowWizard(false); setSelectedRecipe(null); }}
        >
          <div
            className="card"
          ...
          >
            <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 className="card-title" style={{ margin: 0 }}>
                <i className={`bi ${selectedRecipe.icon}`} style={{ marginRight: '0.5rem', color: 'var(--primary-color)' }}></i>
                {selectedRecipe.name}
              </h3>
              <button type="button" className="btn btn-icon btn-secondary" onClick={() => { setShowWizard(false); setSelectedRecipe(null); }}>
                <i className="bi bi-x-lg"></i>
              </button>
            </div>
            <div style={{ padding: '1rem' }}>
              ...
              {selectedRecipe.wizardFields.map((field) => (
                <div key={field.id} className="form-group" style={{ marginBottom: '0.75rem' }}>
                  <label className="form-label">{field.label}</label>
                  <input
                    type={field.type}
                    className="form-control"
                    value={wizardValues[field.id] ?? field.default}
                    onChange={(e) => setWizardValues({ ...wizardValues, [field.id]: field.type === 'number' ? Number(e.target.value) : e.target.value })}
                  />
                </div>
              ))}
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.25rem' }}>
                <button type="button" className="btn btn-secondary" onClick={() => { setShowWizard(false); setSelectedRecipe(null); }}>
                  Cancel
                </button>
                <button type="button" className="btn btn-primary" onClick={applyRecipe}>
                  Build policy
                </button>
              </div>
            </div>
