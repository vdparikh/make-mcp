...
/** One environment profile (dev / staging / prod) for base URLs and env vars */
...
export type EnvProfileKey = 'dev' | 'staging' | 'prod';
export type EnvProfilesMap = Partial<Record<EnvProfileKey, EnvProfile | null>>;
...
export type OutputDisplay = 'json' | 'table' | 'card' | 'image' | 'form' | 'chart' | 'map';

/** Field definition for output_display=form (interactive MCP App form). */
export interface FormFieldConfig {
  name: string;
  label: string;
  type: 'text' | 'textarea' | 'boolean' | 'number' | 'date' | 'time' | 'datetime-local' | 'color';
  default?: unknown;
  required?: boolean;
  placeholder?: string;
}

/** Maps top-level result object keys to MCP Apps card/image fields, or form layout */
export interface OutputDisplayConfig {
  content_key?: string;
  title_key?: string;
  image_url_key?: string;
  /** Target tool name (same server) to call when the form is submitted */
  submit_tool?: string;
  /** Form heading in the MCP App */
  title?: string;
  submit_label?: string;
  fields?: FormFieldConfig[];
  /** Chart widget: bar or line (SVG); tool result must include labels + datasets */
  chart_type?: 'bar' | 'line';
  labels_key?: string;
  datasets_key?: string;
  /** Map widget (Google Maps iframe): lat/lng on result or allowlisted embed URL */
  lat_key?: string;
  lng_key?: string;
  zoom_key?: string;
  embed_url_key?: string;
  /** Default zoom 1–20 when not reading zoom from a result field */
  zoom?: number;
}
...
export interface Tool {
  ...
  output_display_config?: OutputDisplayConfig;
...
export interface PolicyEvaluationResultDetailed extends PolicyEvaluationResult {
  rule_results: RuleEvaluationDetail[];
}
