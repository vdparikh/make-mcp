...
## The governance model (how we frame risk)

Three ideas align Make MCP with **IaaS-style** and **security-leader** expectations:

1. **Identity-aware context** — Hosted MCP can require **Bearer** access to the endpoint and optional **Caller API keys** (`X-Make-MCP-Caller-Id` with keys from **Caller API Keys**) so tool calls are not anonymous LLM traffic; policies and observability can attribute actions to a caller.
2. **Infrastructure portability** — **Docker** is the unit of work for hosted runtimes: reproducible images, resource limits, and “build once, run anywhere” deployment patterns consistent with cloud-native operations.
3. **Audit logging for AI tools** — **Observability** is positioned as an **audit trail**: which tool ran, for whom, success/failure, and latency—so teams can govern agent behavior in production, not only debug it.

For implementation detail, see **Runtime Security Model (Hosted MCP)** below.
...
2. **Turn on caller attribution**
   - Enable **Require caller identity** so clients must send a verified **caller API key** (`mkc_…`) in `X-Make-MCP-Caller-Id` (generate under **Caller API Keys** in the app).
   - The hosted MCP config snippet no longer includes tenant; optional tenant may still be associated with a caller key in storage for reporting, if you use that feature.
...
| **Hosted MCP boundary** | Deploy → Hosted | Bearer / OIDC / mTLS, IP allowlists, caller identity, key rotation, audit export — see [Hosted MCP security](./hosted-security.md). |
