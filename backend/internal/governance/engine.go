...
// RuleEvaluationDetail is the result of evaluating a single rule (for simulation/what-if)
...
// EvaluationResultDetailed adds per-rule results for policy simulation UI
type EvaluationResultDetailed struct {
	Allowed          bool                   `json:"allowed"`
	Reason           string                 `json:"reason,omitempty"`
	ViolatedRules    []string               `json:"violated_rules,omitempty"`
	RequiresApproval bool                   `json:"requires_approval"`
	ApprovalReason   string                 `json:"approval_reason,omitempty"`
	RuleResults      []RuleEvaluationDetail `json:"rule_results"`
}
...
// EvaluatePolicyDetailed returns the same decision as EvaluatePolicy plus per-rule pass/fail details for simulation UI.
func (e *Engine) EvaluatePolicyDetailed(toolID string, input map[string]interface{}, ctx *context.ExtractedContext) *EvaluationResultDetailed {
	summary := e.EvaluatePolicy(toolID, input, ctx)
	out := &EvaluationResultDetailed{
		Allowed:          summary.Allowed,
		Reason:           summary.Reason,
		ViolatedRules:    summary.ViolatedRules,
		RequiresApproval: summary.RequiresApproval,
		ApprovalReason:   summary.ApprovalReason,
		RuleResults:      nil,
	}
	...
	for _, policy := range policies {
		...
		for _, rule := range policy.Rules {
			...
			msg := ""
			if !ruleResult.Allowed {
				msg = ruleResult.Reason
				if ruleResult.RequiresApproval && ruleResult.ApprovalReason != "" {
					msg = ruleResult.ApprovalReason
				}
			}
			out.RuleResults = append(out.RuleResults, RuleEvaluationDetail{
