...
const (
	...
	labelIsolationTier      = "make-mcp.isolation-tier"
)
...
const (
	...
	// Cold start runs npm install + build; do not proxy until HTTP listens.
	hostedHTTPReadyTimeout = 6 * time.Minute
	hostedHTTPReadyPoll    = 400 * time.Millisecond
)
...
// containerBindHost is the Docker port binding HostIP (see config hosted.container_bind_host).
// generatedServerPublicHostIP is passed through to the code generator for hosted snapshots.
func NewManager(db *database.DB, containerBindHost, generatedServerPublicHostIP string) (*Manager, error) {
...
func (m *Manager) EnsureContainer(ctx context.Context, userID string, serverID string, version string, snapshot *models.Server, envVars map[string]string, rt *hostedruntime.Resolved, idleTimeoutMinutes int) (*ContainerConfig, error) {
	...
	cfg, err := m.startContainer(ctx, rootDir, userID, serverID, version, envVars, effectiveIdleTimeout, rt)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) startContainer(ctx context.Context, rootDir string, userID string, serverID string, version string, envVars map[string]string, idleTimeoutMinutes int, rt *hostedruntime.Resolved) (*ContainerConfig, error) {
	if rt == nil {
		def, err := hostedruntime.Resolve(hostedruntime.UserConfig{}, hostedruntime.DefaultPlatformLimits())
		if err != nil {
			return nil, err
	...
	hostPort := randomPort()
	pidsLimit := rt.PidsLimit
	memLimit := rt.MemoryBytes
	nanoCPUs := rt.NanoCPUs
	...
	if err := m.waitForHostedHTTPReady(ctx, hostPort); err != nil {
		...
		defer cancel()
		t := 10
		_ = m.cli.ContainerStop(stopCtx, resp.ID, container.StopOptions{Timeout: &t})
		_ = m.cli.ContainerRemove(stopCtx, resp.ID, container.RemoveOptions{Force: true})
		return nil, err
	}
	...
	manifest := HostedManifest{
		...
		Metadata: map[string]interface{}{
			...
			"isolation": map[string]interface{}{
				"tier":          rt.Tier,
				"egress_policy": rt.EgressPolicy,
				"egress_hosts":  rt.EgressAllowlist,
			},
			"resources": map[string]interface{}{
				"memory_mb":  rt.MemoryBytes / (1024 * 1024),
				"nano_cpus":  rt.NanoCPUs,
				"pids_limit": rt.PidsLimit,
			},
