...
async function reportObservabilityEvent(
  ...
  try {
    ...
    await fetch(MCP_OBSERVABILITY_ENDPOINT, {
      ...
      body: JSON.stringify({
        key: MCP_OBSERVABILITY_KEY,
        events: [{ tool_name: toolName, duration_ms: durationMs, success, error: errorMessage, repair_suggestion: repairSuggestion }],
      }),
...
To send tool calls, latency, and failures to Make MCP (so you can see them in the Observability tab), set:
